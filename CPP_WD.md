...
- 基类对象与子类对象的相互转换: `Base base = derive;`, `Base * base = &derive;`, `Base & base = derive;`,向上转换直接可以,反之不能直接转换,也不一定能转换成功.
- polymorphic 多态的
- 多态是: 通过基类指针/引用调用虚函数时，能自动执行派生类的版本.
- 继承derivew是多态polymorphism的基础. 基类指针指向派生类对象，并且函数是虚函数.基类虚函数virtual 是触发多态行为的开关.
