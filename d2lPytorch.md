...
- 方差是数据偏离均值的平方的均值,是单位的平方,标准差是方差的开方,回归原始单位, 不清楚用哪个可以用量纲分析
#### tensor
- tensor创建方法:
...
- 缺失值处理方式:插值法/删除法, pands的`pd.fillna(xx)`会识别数据中的`NaN`或者`NA`(本质是np.nan), 填写指定的内容,如均值,中位数,前向填充
- dummy 假的,虚拟的,傀儡,哑巴
...
#### 乘法
- 向量与向量按元素乘法: `x * y`
- 矩阵按元素乘法成为哈达玛积(不是点乘哈,点乘还要相加) :`A * B`
...
## 深度学习计算
  ...
  - 待操作的两个张量对象必须在同一设备上
  - 来回复制: `tensor_cpu = tensor_cuda.cpu()`, `tensor_cuda = tensor_cpu.cuda(0)`
  - `net.to("cuda")`
  ## 卷积神经网络(cnn)
- 归纳偏置是算法对问题本质的预设，是设计者强加给模型的先验知识。
  - CNN 的归纳偏置
      - 局部性：相邻像素有关联
      - 平移不变性：模式在任何位置都一样
      - 层次性：低级特征组合成高级特征
...
#### 卷积conv
- 卷积/滤波: 识别相邻元素间的相互作用
- 通道/特征映射: 每个通道学习到不同的特征
- torch中的卷积层(convolution)进行的其实是互相关计算(cross-correlation),而不是数学上的卷积运算(把卷积核（比如 3×3 的小矩阵）上下左右翻转 180°（也叫 “旋转 180°”）),因为卷积核元素本身就是参数,学习出来的效果和真正的卷积一样的,还少了旋转这一步
- `Y[i,j] = (X[i:i+h, j:j+w] * K).sum() `, 逐元素相乘
- 对于某一层的任意元素x，其感受野（receptivefield）是指在前向传播期间可能影响x计算的所有元素（来自所有先前层）
- 输入形状为nh × nw，卷积核形状为kh × kw，那么输出形状将是(nh − kh + 1) × (nw − kw + 1)
- 填充(padding): 在应用了连续的卷积之后，我们最终得到的输出远小于输入大小,原始图像的边界丢失了许多有用信息。而填充是解决此问题最有效的方法
- 步幅(stride): 有时，我们可能希望大幅降低图像的宽度和高度。例如，如果我们发现原始的输入分辨率十分冗余。步幅则可以在这类情况下提供帮助,每次滑动元素的数量叫做步幅
- $\lceil x/2 \rceil$ 天花板取整
- $\lfloor x/2 \rfloor$ 地板取整
- `conv2d = nn.Conv2d(1, 1, kernel_size=3, padding=1)` 所有侧边填充1, 默认填充0元素
- `conv2d = nn.Conv2d(1, 1, kernel_size=(5, 3), padding=(2, 1))`
- `conv2d = nn.Conv2d(1, 1, kernel_size=(3, 5), padding=(0, 1), stride=(3, 4))`
- 5 输入通道 → 每个输出通道需要 5 个卷积核，分别和 5 个输入通道做运算后求和, 得到一个输出通道
- 1x1卷积核,加权求和输入通道, 调整输出通道的数量, 跨通道信息融合, 不改变输入数据的空间特征结构, (代替线性)
- `torch.matmul` 是通用矩阵乘法,支持高维张量,支持广播机制; `torch.mm` 是2维矩阵专用乘法
- 假设我们有两个卷积核，大小分别为k1和k2（中间没有非线性激活函数）,运算可以用单次卷积来表示。这个等效的单个卷积核的维数是out = k1 + k2 - 1
#### 池化pooling
- 最大池化层(maximum pooling), 平均池化层(average pooling)
- 作用:展展开开 
  1.降维,减少计算量
  2.增强平移不变性
  3.增大感受野
## 现代卷积神经网络结构
- 在卷积神经网络中，我们组合使用卷积层、非线性激活函数和汇聚层。• 为了构造高性能的卷积神经网络，我们通常对卷积层进行排列，逐渐降低其表示的空间分辨率，同时增加通道数。• 在传统的卷积神经网络中，卷积块编码得到的表征在输出之前需由一个或多个全连接层进行处理
...
- `flat = nn.Flatten(start_dim=1, end_dim=-1)` 默认保留第零个维度,把第一个到最后一个相乘
...
- `nn.train() ` 和 `nn.eval()`行为区别:
  ...
  | 层类型 | 训练模式 | 评估模式 |
  | --------------- | --------------- | --------------- |
  | dropout层 | 随机丢弃神经元 | 保留所有 |
  | batchNorm层 | 当前batch数据均值和方差 | 训练集所有数据均值和方差 |
