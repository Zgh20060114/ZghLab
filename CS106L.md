...
- 初始化的方式：
  ...
  - uniform initialization 统一初始化/列表初始化`int a{1};  Point p{1,2}`
...
- 默认的，类的继承是私有的
- slice 切片
- __切片问题__: 把派生类对象赋值给基类对象，拷贝时会发生切片问题：派生类对象直接变成基类对象，新增的重写的全没了
- 所以解决对象切片问题的办法就是避免拷贝，使用指针
  - ![避免拷贝切换](./assets_CS106L/1.png)
- ![继承方式权限](assets_CS106L/2026-02-14-17-14-05.png)
- dispatch 分发分配，调度
...
- crash 碰撞，崩溃
- crash course 速成课
- decltype 声明类型,用于推断出一个变量或者表达式的精准原始类型(无类型退化)
- std::type_identity<decltype(s)>::type 返回s本身的类型本身
- typeid(std::type_identity<decltype(s)>::type).name() 返回类型的字符串
- spdlog, proxy模版元编程, 
- clang,mscv,g++(gcc针对cpp的分支)编译器
- clangd 是语言服务器,辅助代码理解和补全等
- 解决无法识别问题的标准方法是生成一个名为 compile_commands.json 的文件。这个文件被称为“编译数据库”（Compilation Database），它精确记录了项目中每个源文件是如何被编译的，包括所有的编译选项、宏定义和头文件搜索路径。Clangd 读取这个文件后，就能以和实际构建完全相同的环境来解析代码，从而消除所有误报
- delete 释放单个元素内存; delete[] 释放数组所有元素内存;new和new[]同理.
- new delete; new[] delete[]
~~~cpp
  void read_only(const Foo& obj)
  {
      std::cout << obj.val;
  }
  Foo stack_obj;
  read_only(stack_obj);
  auto heap_obj = std::make_unique<Foo>(5);
  read_only(*heap_obj); // 解引用智能指针传入
~~~
- 函数需要传递指针的时候,用XX &ptr/ const XX &ptr, 调用的时候用*解引用
- upon = on 
- thread safe:一段代码 / 类具备线程安全:多线程并发同时调用其公开成员函数时，无需调用者额外加锁，内部同步机制可保证数据无竞争、不会出现内存损坏、逻辑错乱、数据丢失、崩溃等未定义行为。
